...
def gamma_linha_x(t):
    return -5*np.sin(t) + 10*np.sin(5*t/2)
...
for t1 in t_values:
        if (gamma_linha_x(t1)<0.01) and (gamma_linha_x(t1)>-0.01):
...
for i in (range(len(intersectionst1))):
    print("tangencia vertical em t = "+ str(intersectionst1[i]))
